...
contract("Truth", async (accounts) => {
  
    const owner=accounts[0];
    const user=accounts[1];

    beforeEach(async () => {
        instance = await truth.deployed()
    });
    ...
    it("Should Create a Truth", async () => {
        let hashtag='check';
        let content='checkName';
        let imghash='checkImg';
        await instance.createTruth(hashtag,content,imghash,{from:user});
        let nooftruths=await instance.totalTruths(); 
        assert.equal(1,nooftruths, "Test failed, value was not same");
    });
