App = {
  ...
  loadUserProfile: async () => {
    ...
    if (App.userStatus == 0) {
        ...
        if(password === "123456789") {
          ...
          reader1.onloadend = async function () {
            ...
            reader2.onloadend = async function () {
              ...
              await App.contracts.truth.methods.registerUser($("#username").val(), $("#name").val(), result1[0].hash, result2[0].hash, $("#bio").val()).send({ from: App.account });
      ...
      App.user = await App.contracts.truth.methods.getUser().call({ from: App.account });
